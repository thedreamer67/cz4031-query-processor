...
class QueryPlans:
  ...
  def extract_qp_data(self, json_obj):
    ...
    Args:
        json_obj (json): QP generated from EXPLAIN (FORMAT JSON) query
    Returns:
    ...
    while not q_child_plans.empty():
      ...
      if "Scan" in current_node.node_type:
        if "Index" in current_node.node_type:
          if relation_name:
            current_node.write_qp_output_name("{} with index {}".format(relation_name, index_name))
        elif "Subquery" in current_node.node_type:
          current_node.write_qp_output_name(alias)
        else:
          current_node.write_qp_output_name(relation_name)
